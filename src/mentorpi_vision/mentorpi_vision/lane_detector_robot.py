...
class ProcessFrame(Node):
    def __init__(self):
        ...
        self.last_time = time.time()
        self.fps = 0.0
        self.get_logger().info('Wizja odpalona, Zastosowano algorytm Hough + FIR (Czarne linie proste).')
    ...
    def perform_detection(self, frame):
        # ZMIANA: Wywołanie funkcji detekcji czerni
        left_lines, right_lines = self.detect_black_lines(frame)
    ...
    def detect_black_lines(self, frame):
        ...
        # ZMIANA: Definicja koloru czarnego w HSV z odpowiednią tolerancją
    ...
    def fit_and_filter(self, lines, history, missing_counter):
        ...
        # ZMIANA: Minimum 2 punkty potrzebne do wykreślenia prostej
        if len(np.unique(y_coords)) < 2:
            return None, missing_counter

        # ZMIANA: Dopasowanie prostej (1 stopień) zamiast paraboli
        poly = np.polyfit(y_coords, x_coords, 1)

        history.append(poly)

        if len(history) == 7:
            # ZMIANA: Tablica o rozmiarze 2 dla współczynników linii (a, b)
    ...
    def calculate_and_log(self, frame_shape, left_poly, right_poly):
        ...
        if left_poly is not None and right_poly is not None:
            ...
            # ZMIANA: Obliczanie pozycji X na podstawie równania prostej (x = a*y + b)
            left_x = left_poly[0]*lookahead_y + left_poly[1]
            right_x = right_poly[0]*lookahead_y + right_poly[1]
