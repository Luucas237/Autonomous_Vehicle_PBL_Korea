...
class ProcessFrame(Node):
    def __init__(self):
        ...
        self.fps = 0.0
        self.get_logger().info('Wizja odpalona! Czekam na strumień wideo...')
    ...
    def perform_detection(self, frame):
        ...
        cv.imshow("Maska - Region of Interest", roi_debug)
        cv.imshow("Podglad z Robota - Linie", output)
    ...
    def draw_guideline(self, frame, left_poly, right_poly):
        ...
        if left_poly is not None and right_poly is not None:
            left_x1, left_x2 = int(left_poly[0] * y1 + left_poly[1]), int(left_poly[0] * y2 + left_poly[1])
            right_x1, right_x2 = int(right_poly[0] * y1 + right_poly[1]), int(right_poly[0] * y2 + right_poly[1])

            mid_x1 = (left_x1 + right_x1) // 2
            mid_x2 = (left_x2 + right_x2) // 2

            cv.line(frame, (left_x1, y1), (left_x2, y2), (255, 0, 0), 4)    
            cv.line(frame, (right_x1, y1), (right_x2, y2), (255, 0, 0), 4)  
            cv.line(frame, (mid_x1, y1), (mid_x2, y2), (0, 0, 255), 3)      

            offset = mid_x2 - (width // 2)
